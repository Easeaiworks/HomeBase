...
function ActionCard({ action }: { action: AIToolResult }) {
  const iconMap: Record<string, string> = {
    create_event: 'Ã°ÂÂÂ',
    add_grocery_item: 'Ã°ÂÂÂ',
    log_expense: 'Ã°ÂÂÂ°',
    draft_message: 'Ã¢ÂÂÃ¯Â¸Â',
    search_recipes: 'Ã°ÂÂÂ³',
    add_maintenance_item: 'Ã°ÂÂÂ§',
    find_local_service: 'Ã°ÂÂÂª',
    check_schedule_conflicts: 'Ã¢ÂÂ°',
    get_budget_status: 'Ã°ÂÂÂ',
    store_receipt: 'Ã°ÂÂ§Â¾',
    search_receipts: 'Ã°ÂÂÂ',
  };

  const icon = iconMap[action.action] || 'Ã¢ÂÂ';
      ...
      <View style={styles.actionContent}>
        ...
        {action.result?.budget_warning && (
          <View style={styles.budgetWarning}>
            <Text style={styles.budgetWarningText}>Ã¢ÂÂ Ã¯Â¸Â {action.result.budget_warning}</Text>
          </View>
        )}
  ...
  return (
    <Animated.View style={{ transform: [{ scale: pulseAnim }] }}>
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        <Text style={styles.micEmoji}>{isListening ? 'Ã°ÂÂÂ´' : 'Ã°ÂÂÂÃ¯Â¸Â'}</Text>
      </TouchableOpacity>
    </Animated.View>
  );
}
...
export default function VoiceAssistantScreen() {
  ...
  // Quick action chips
  const quickActions = [
    { label: "What's on my schedule?", icon: 'Ã°ÂÂÂ' },
    { label: 'Add to grocery list', icon: 'Ã°ÂÂÂ' },
    { label: "What's for dinner?", icon: 'Ã°ÂÂÂ³' },
    { label: 'Check my budget', icon: 'Ã°ÂÂÂ°' },
  ];
      ...
      >
        ...
        >
          {messages.length === 0 && (
            <View style={styles.welcomeContainer}>
              <Text style={styles.welcomeEmoji}>Ã°ÂÂÂ</Text>
        ...
        {isListening && (
          <View style={styles.listeningBanner}>
            <Text style={styles.listeningDot}>Ã°ÂÂÂ´</Text>
          ...
          <View style={styles.inputRow}>
            ...
            {inputText.trim() && !isListening ? (
              <TouchableOpacity
                ...
                disabled={isLoading}
              >
                <Text style={styles.sendEmoji}>Ã¢ÂÂ</Text>
              </TouchableOpacity>
            ) : (
